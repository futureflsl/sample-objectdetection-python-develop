...
function upload_path()
{
    ...
    ret=`IDE-daemon-client --host ${remote_host}:${remote_port} --hostcmd "mkdir -p ~/HIAI_PROJECTS/ascend_workspace/pythonClassifyDemo/resnet18Result"`
    return 0
}
...
function deploy_app()
{
    if [ -d ${script_path}/pythonClassifyDemo ];then
        echo "Deploy app..."
        iRet=`IDE-daemon-client --host ${remote_host}:${remote_port} --hostcmd "rm -rf ~/HIAI_PROJECTS/ascend_workspace/pythonClassifyDemo"`
        if [[ $? -ne 0 ]];then
            echo "ERROR: delete ${remote_host}:./pythonClassifyDemo failed, please check /var/log/syslog for details."
            return 1
        fi
        upload_path ${script_path}/pythonClassifyDemo "~/HIAI_PROJECTS/ascend_workspace/pythonClassifyDemo"
        if [[ $? -ne 0 ]];then
            echo "ERROR: upload pythonClassifyDemo to developer kit failed!"
            return 1
        fi
    fi
}
...
main()
{
    ...
    echo "Finish to deploy classificationDemoPython."
    exit 0
}
